...
launch_args = [
    ...
    DeclareLaunchArgument('gui', default_value='/root/ws/src/jackal_nav/usd/office.usd', description='Provide the path to a usd file to open it when starting Isaac Sim in standard gui mode. If left empty, Isaac Sim will open an empty stage in standard gui mode.'),
    ...
    DeclareLaunchArgument('headless', default_value='webrtc', description='Set to "native" or "webrtc" to run Isaac Sim with different headless modes, if left blank, Isaac Sim will run in the regular GUI workflow. This parameter can be overridden by "standalone" parameter.'),
...
def launch_setup(context):
    ...
    isaacsim_node = Node(
        package='jackal_nav', executable='run_isaacsim.py',
