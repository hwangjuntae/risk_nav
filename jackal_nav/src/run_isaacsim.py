...
class IsaacSimLauncherNode(Node):
    ...
    def execute_launch(self):
        ...
        if args.install_path != "":
            ...
            if version_ge(args.version, "4.2.0") and not version_gt(args.version, "2021.2.0"):
                if sys.platform == "win32":
                    ...
                    filepath_root = os.path.join(home_path, "isaac-sim")
        ...
        # Allow running as root user for Isaac Sim
        ...
        if args.standalone != "":
            ...
            # Default command
            executable_command = f'{os.path.join(filepath_root, "isaac-sim.sh" if sys.platform != "win32" else "isaac-sim.bat")} --/isaac/startup/ros_bridge_extension=isaacsim.ros2.bridge --allow-root'

            if args.headless == "webrtc":
                executable_command = f'{os.path.join(filepath_root, "isaac-sim.streaming.sh" if sys.platform != "win32" else "isaac-sim.streaming.bat")} --/isaac/startup/ros_bridge_extension=isaacsim.ros2.bridge --allow-root'
